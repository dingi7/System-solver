function initArray() {
  ...
  for (let i = 0; i < this.length; i++) this[i + 1] = initArray.arguments[i];
}
...
function solve() {
  ...
  if (a1[1] == 0) {
    alert(
      "Уверете се, че първият елемент на първия ред е различен от нула."
    );
  }
  ...
  if (
    t1[1] * a1[4] + t1[2] * a2[4] + t1[3] * a3[4] != t1[4] ||
    t3[1] * a1[4] + t3[2] * a2[4] + t3[3] * a3[4] != t3[4] ||
    t2[1] * a1[4] + t2[2] * a2[4] + t2[3] * a3[4] != t2[4]
  ) {
    alert(
      "Няма решение. Моля, проверете входните данни."
    );
    return;
  }else{
